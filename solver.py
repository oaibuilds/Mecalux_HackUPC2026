...
class BayType:
    ...
    def efficiency(self, wh_area: float) -> float:
        return self.price / (self.n_loads ** 2) - self.area / wh_area
...
class PlacedBay:
    ...
    def to_dict(self) -> dict:
        ...
        gp_coords = (list(self.gap_poly.exterior.coords)
                     if self.gap_poly and not self.gap_poly.is_empty else [])
        return {
            "id":       self.type_id,
            "x":        round(self.x, 2),
            "y":        round(self.y, 2),
...
def get_ceiling_at(x, ceiling):
    if not ceiling: return float('inf')
    if x <= ceiling[0][0]: return ceiling[0][1]
    if x >= ceiling[-1][0]: return ceiling[-1][1]
    for i in range(len(ceiling) - 1):
        x0, h0 = ceiling[i]; x1, h1 = ceiling[i+1]
        if x0 <= x <= x1:
...
def min_ceiling_over_bounds(minx, maxx, ceiling, steps=10):
    if not ceiling: return float('inf')
    return min(get_ceiling_at(minx + (maxx-minx)*i/steps, ceiling) for i in range(steps+1))
...
def real_overlap(a, b, a_b, b_b):
    if not bounds_overlap(a_b, b_b): return False
    if not a.intersects(b): return False
...
class WarehouseSolver:
    def __init__(self, warehouse_vertices, obstacles, ceiling, bay_types):
        ...
        self.bay_types = [
            BayType(id=int(b[0]), w=b[1], d=b[2], h=b[3],
                    gap=b[4], n_loads=int(b[5]), price=b[6])
            for b in bay_types
        ]
    ...
    def can_place(self, x, y, bt, angle, placed):
        ...
        if not self.prepared.contains(fp): return None
        if min_ceiling_over_bounds(fp_b[0], fp_b[2], self.ceiling) < bt.h: return None

        for poly, bds in zip(self.obstacle_polys, self.obstacle_bounds):
            if real_overlap(fp, poly, fp_b, bds): return None

        for p in placed:
            if real_overlap(fp, p.footprint, fp_b, p.fp_bounds): return None
            if p.gap_poly and not p.gap_poly.is_empty:
                if real_overlap(fp, p.gap_poly, fp_b, p.gap_poly.bounds): return None

        if bt.gap > 0 and not gp.is_empty:
            gp_b = gp.bounds
            if not self.prepared.contains(gp): return None
            for poly, bds in zip(self.obstacle_polys, self.obstacle_bounds):
                if real_overlap(gp, poly, gp_b, bds): return None
            for p in placed:
                if real_overlap(gp, p.footprint, gp_b, p.fp_bounds): return None
    ...
    def _do_place(self, x, y, bt, angle, placed):
        ...
        if result:
            ...
            placed.append(PlacedBay(
                type_id=bt.id, x=x, y=y, rotation=angle,
                w=bt.w, d=bt.d, h=bt.h,
                n_loads=bt.n_loads, price=bt.price, gap=bt.gap,
                footprint=fp, gap_poly=gp, fp_bounds=fp.bounds,
            ))
            return True
        return False
    ...
    def compute_score(self, placed):
        if not placed:
            return {"score":0,"totalBays":0,"totalLoads":0,"totalArea":0,
                    "warehouseArea":self.wh_area,"areaUsage":0,"totalCost":0}
        sum_pl = sum(b.price / (b.n_loads**2) for b in placed)
        sum_a  = sum(b.area for b in placed)
        return {
            "score": sum_pl - sum_a / self.wh_area,
            "totalBays": len(placed),
            "totalLoads": sum(b.n_loads for b in placed),
            "totalArea": sum_a,
            "warehouseArea": self.wh_area,
            "areaUsage": sum_a / self.wh_area * 100,
    ...
    def solve(self, time_limit=25.0, extra_angles=None):
        ...
        for bt, ang in candidates:
            if time.time() - start > time_limit * 0.65: break
            y = self.min_y
            while y < self.max_y:
                x = self.min_x
                while x < self.max_x:
        ...
        if time.time() - start < time_limit * 0.85:
            ...
            for p in placed:
                ...
                if p.gap_poly and not p.gap_poly.is_empty:
                    ...
                    xs.update([gb[0], gb[2]]); ys.update([gb[1], gb[3]])
            for o in self.obstacles:
                xs.update([o[0], o[0]+o[2]]); ys.update([o[1], o[1]+o[3]])
            for v in self.vertices:
                xs.add(v[0]); ys.add(v[1])

            for bt, ang in candidates:
                if time.time() - start > time_limit * 0.95: break
                for y in sorted(ys):
                    for x in sorted(xs):
                        self._do_place(x, y, bt, ang, placed)
...
def parse_warehouse(text):
    return [tuple(map(float, l.split(","))) for l in text.strip().splitlines()
